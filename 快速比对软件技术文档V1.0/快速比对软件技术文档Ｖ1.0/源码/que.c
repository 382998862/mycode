...
{  
    ...
    if(pnode != NULL) {  
        ...
        pthread_mutex_lock(&pqueue->q_lock); //与出队互斥,加锁 

        if(size >0)
            pthread_cond_signal(&pquenue->q_clock);   
        if(IsEmpty(pqueue)) {  
            pqueue->front = pnode;  
        } 
        else {  
        ...
        pqueue->rear = pnode;  
        pqueue->size++;  //加条件变量 限制缓冲队列的大小
       // pthread_cond_signal(&pqueue->cond);  
        pthread_mutex_unlock(&pqueue->q_lock);  //解锁
    }  
    return pnode;  
}  
/*队头元素出队*/  
...
{  
    ...
    pthread_mutex_lock(&pqueue->q_lock);    // 枷锁
    while(size <= 0)
        pthread_cond_wait(&pqueue->q_cond,&pqueue->q_lock);  
    if(!IsEmpty(pqueue)) {  
        pqueue->size--;       //加条件变量缓冲队列空时堵塞 
