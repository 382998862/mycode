...
my %opts;
GetOptions(\%opts,"i=s","o=s","s=s","r=s","od=s","n=s","m=s","mode=s","min=s","max=s","h" );

#&help()if(defined $opts{h});
if(!defined($opts{i}) || !defined($opts{s}) || !defined($opts{od}) || !defined($opts{mode}) || !defined($opts{r}) || !defined($opts{o}) || defined($opts{h}))
{
	print <<"	Usage End.";
	Description:
...
my $gatk = "/share/nas2/genome/biosoft/GATK/3.0-0/GenomeAnalysisTK.jar" ;
my $snpEff = "/share/nas1/dengdj/bin/Bio_soft/snpEff/v3_6/snpEff/snpEff.jar" ;
my $sEffConfig="/share/nas12/wujh/Pipeline/Reseq/Prepare/Analysis_Preparation/Database/snpEff.config";
#/share/nas1/dengdj/bin/Bio_soft/snpEff/snpEff.jar
...
sub get_sample_files()
{
	...
	while(<IN>){
		...
		print SH "$java_dir/java -jar $gatk -T SelectVariants -R $reffile --variant $vcffile -o $outfile -sn $sample -env \n" ;
...
sub annotation_for_variants()
{
	...
	$cmd = "cd $outdir && $java_dir/java -Xmx10240m -jar $snpEff $species -v $vcffile -c $sEffConfig -o gatk > $outfile" ;
	&run_or_die($cmd);
...
#	$cmd = "$java_dir/java -jar $gatk -T CombineVariants -R $reffile -nt 8 -o $outfile";
...
#	&run_or_die($cmd);
	...
	if ($mode =~ /Indel/i){
		...
		$cmd = "perl $Bin/../SNPCalling/indel_stat_v1.1.pl -i $anno_file -o $out_stat_file -m $min_length -x $max_length" ;
		&run_or_die($cmd);
	}
