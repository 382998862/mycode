...
my %opts;
GetOptions(\%opts,"r=s","od=s","id=s","region=s","ploidy=s","p=s","t=s","cpu=s","db=s","m=s","h","dedup","indel","GATK" ); #20160621"dedup","indel   20160719 GATK
...
if(!defined($opts{r}) || !defined($opts{p}) || !defined($opts{id}) || !defined($opts{db}) || !defined($opts{od}) || !defined($opts{region}) || defined($opts{h}))
{
	...
	Optional parameters:
		...
		-dedup       markduplication and delete   [str]
		-indel       call indel                   [str]
		-GATK		 use GATK for SNPcalling      [str]
		-h           Help document
...
mkdir $tmp_dir ;
my @bamfiles = () ;
my $picard_dir = "/share/nas2/genome/biosoft/picard-tools/current/" ;  ### 20160603
my $gatk_dir="/share/nas2/genome/biosoft/GATK/3.0-0/";  ### 20160603
my $java_dir="/share/nas1/dengdj/bin/Bio_soft/jre1.7.0_45/bin" ;    ###!!!!!! 
my $bcftools_dir = "/share/nas2/genome/biosoft/samtools-0.1.18/bcftools/" ; ####!!!!nas2 none
my $mark_dup =  $opts{dedup};
my $call_indel = $opts{indel};
my $call_indel_para;
if (defined $call_indel) {
	$call_indel_para = "-glm BOTH";
}
else{
	$call_indel_para = "";
}
my $use_GATK = $opts{GATK};  #20160719
...
my (@dup_bam_files,@realn_bam_files);
...
if (defined $mark_dup ){
	...
	## Local realignment
	&show_log("#------------ Start Local realignment...") ;
	@realn_bam_files = &local_realignment(\@bamfiles);
	&show_log("#------------ Local realignment done.") ;	
}
...
my (@bqsr_bam_files) = &base_quality_score_recalibration(\@realn_bam_files);   ###!!!
...
if (defined $use_GATK ) {
	...
	@variant_files = &hard_filter(\@variant_files);
	&show_log("#------------ Filtering done...");
}
else{
	## Variant calling varscan
	&show_log("#------------ Start variants calling...(Varscan)") ;
	my @variant_files = &Varscan_variant_calling(\@bqsr_bam_files);
	&show_log("#------------ Variant calling done.") ;
}
...
sub index_fa()
{
	...
	if (!-f "$dict_file"){
		my $cmd = "java -Djava.io.tmpdir=$tmp_dir -jar $picard_dir/CreateSequenceDictionary.jar REFERENCE=$ref OUTPUT=$dict_file" ;
		&run_or_die($cmd);
	}
	my $fa_length_file = "$ref.len" ;
	my $dir = dirname($ref);
	if (!-f $fa_length_file){
		my $cmd = "perl $Bin/../Tools/ref_GC_len.pl -ref $ref -od $dir" ;
		&run_or_die($cmd);
	}
...
sub duplicate_marking()
{
	...
	for my $sample (sort keys %hsamples_bam){
		...
		print SH "java -Djava.io.tmpdir=$tmp_dir -jar $picard_dir/MarkDuplicates.jar MAX_FILE_HANDLES_FOR_READ_ENDS_MAP=512 VALIDATION_STRINGENCY=LENIENT REMOVE_DUPLICATES=true" ;  ##20160614 delete duplication
...
sub local_realignment()
{
	...
	for my $bamfile (@{$abamfiles}){
		...
		print SH "java -Djava.io.tmpdir=$tmp_dir -jar $gatk_dir/GenomeAnalysisTK.jar -T RealignerTargetCreator -I $bamfile -R $ref -o $intervals_file -L $region -rf BadCigar \n" ; # 20160621 badcigar
	}
#	&qsub($interval_file);
	&sh($interval_file);
	close(SH);
	...
	for my $bamfile (@{$abamfiles}){
		...
		print SH "java -Djava.io.tmpdir=$tmp_dir -jar $gatk_dir/GenomeAnalysisTK.jar -T IndelRealigner -I $bamfile -R $ref -L $region -targetIntervals $intervals_file -o $outfile  \n" ;
...
sub generate_grp_file()
{
	...
	for my $bamfile (@{$abamfiles}) {
		...
		print SH "java -Djava.io.tmpdir=$tmp_dir -jar $gatk_dir/GenomeAnalysisTK.jar -T BaseRecalibrator -R $ref -I $bamfile -L $region -knownSites $db_vcf_file -mte -nct $thread_num -o $outfile \n";
...
sub generate_recalibration_plots()
{
	...
	#my $cmd = "java -Djava.io.tmpdir=$tmp_dir -jar $gatk_dir/GenomeAnalysisTK.jar -T AnalyzeCovariates -R $ref" .
...
sub recalibration_for_each_sample()
{
	...
	foreach my $i (0..($len-1)){
		...
		print SH "java -Djava.io.tmpdir=$tmp_dir -jar $gatk_dir/GenomeAnalysisTK.jar -T PrintReads -R $ref -L $region -BQSR $tables[$i] -I $bams[$i] -o $outfile \n" ;
		push (@bqsr_bam_files, $outfile);
	}
	close(SH);
#	&qsub($shfile, "middle.q");     #0217
#	&qsub($shfile);
	&sh($shfile);
	return (@bqsr_bam_files);
}
...
sub variants_calling_by_gatk()
{
	...
	for my $bamfile (@{$abamfiles}) {
		...
		my $cmd = "java -Djava.io.tmpdir=$tmp_dir -Xmx20G -jar $gatk_dir/GenomeAnalysisTK.jar -T UnifiedGenotyper  $call_indel_para -I $bamfile -L $region -R $ref -nct $thread_num -dt NONE --dbsnp $db_vcf_file -o $gatk_vcf \n" ; ###20160614 !!!
		print SH $cmd;
...
###Varscan_variant_calling					20160719

sub Varscan_variant_calling ()
{
	my ($abamfiles) = @_ ;
	my $dir = "$outdir/variant_calling";
	mkdir "$dir";
	my @varscan_vcf_files;
	my $shfile = "$dir/$outprefix.ug.sh" ;
	open(SH,">$shfile")|| die "Can't creat $shfile, $!\n" ; 	  #
	for my $bamfile (@{$abamfiles}) {
		my $basename = basename($bamfile);
		(my $varscan_vcf = "$dir/$basename") =~ s/bam$/ug.vcf/;
		(my $mpileup_out = "$dir/$basename") =~ s/bam$/mpileup.out/;
		my $cmd = "samtools mpileup  -f $ref -l $region -d 3000 -Q 17 -q 17  $mpileup_out  > $varscan_vcf \n java -jar /share/nas2/genome/biosoft/VarScan/VarScan.v2.3.9.jar mpileup2cns $mpileup_out --output-vcf 1 -min-var-freq 0.001 --variants > $varscan_vcf";
		print SH $cmd;
		push (@varscan_vcf_files,$varscan_vcf);
	}
	close(SH);
	&sh($shfile);
	return(@varscan_vcf_files);
}
...
sub hard_filter()
{
	...
	for my $vcf (@{$gatk_vcf}) {
		...
		print SH "java -jar $gatk_dir/GenomeAnalysisTK.jar -T SelectVariants -R $ref --variant $vcf -o $filter -select \'DP > 20\'", "\n";
...
sub Annotate()
{
	...
	for my $vcf (@{$vcffiles}) {
		...
		print "perl $Bin/SNPAnnotation_v1.2.pl -i $vcf -od $outdir/Anno -o $outp -r $ref -s Human -mode SNP \n";
