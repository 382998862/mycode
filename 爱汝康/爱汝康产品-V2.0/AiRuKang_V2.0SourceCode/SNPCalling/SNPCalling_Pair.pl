...
sub variants_calling_by_gatk()
{
	...
	my $shfile = "$dir/$outprefix.haplotypecaller.sh" ;
	open (SH, ">$shfile") || die "Can't creat $shfile, $!\n" ; 
	print "thread is $thread_num !!!!!!!!!!!";
	for my $bamfile (@{$abamfiles}) {
		my $basename = basename($bamfile);
		(my $gatk_vcf = "$dir/$basename") =~ s/bam$/hc.vcf/;
		(my $reassemble_bam = "$reassemble_dir/$basename") =~ s/bam$/reassemble.bam/;
		my $cmd = "$java_dir/java -Djava.io.tmpdir=$tmp_dir -Xmx20G -jar $gatk_dir/GenomeAnalysisTK.jar -T HaplotypeCaller -I $bamfile -L $region -R $ref -nct $thread_num -dt NONE --dbsnp $db_vcf_file -bamout $reassemble_bam --bamWriterType ALL_POSSIBLE_HAPLOTYPES -o $gatk_vcf \n" ;
		print SH $cmd;
