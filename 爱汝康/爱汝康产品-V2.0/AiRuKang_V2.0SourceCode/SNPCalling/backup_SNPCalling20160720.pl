...
my %opts;
GetOptions(\%opts,"r=s","od=s","id=s","region=s","ploidy=s","p=s","t=s","cpu=s","db=s","m=s","h","dedup","indel" ); #20160621"dedup","indel
...
sub generate_grp_file()
{
	...
	for my $bamfile (@{$abamfiles}) {
		...
		print SH "java -Djava.io.tmpdir=$tmp_dir -jar $gatk_dir/GenomeAnalysisTK.jar -T BaseRecalibrator -R $ref -I $bamfile -L $region -knownSites $db_vcf_file -mte -nct $thread_num -o $outfile -rf BadCigar \n";   #20160622 BQSRbadcigar
...
sub variants_calling_by_gatk()
{
	...
	for my $bamfile (@{$abamfiles}) {
		...
		my $cmd = "java -Djava.io.tmpdir=$tmp_dir -Xmx20G -jar $gatk_dir/GenomeAnalysisTK.jar -T UnifiedGenotyper -glm BOTH -I $bamfile -L $region -R $ref -nct $thread_num -dt NONE --dbsnp $db_vcf_file -o $gatk_vcf -rf BadCigar  \n" ; ###20160614
		print SH $cmd;
