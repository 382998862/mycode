...
sub duplicate_marking()
{
	...
	for my $sample (sort keys %hsamples_bam){
		...
		print SH "java -Djava.io.tmpdir=$tmp_dir -jar $picard_dir/MarkDuplicates.jar MAX_FILE_HANDLES_FOR_READ_ENDS_MAP=512 VALIDATION_STRINGENCY=LENIENT" ;
...
sub variants_calling_by_gatk()
{
	...
	for my $bamfile (@{$abamfiles}) {
		...
		my $cmd = "java -Djava.io.tmpdir=$tmp_dir -Xmx20G -jar $gatk_dir/GenomeAnalysisTK.jar -T UnifiedGenotyper -I $bamfile -L $region -R $ref -nct $thread_num -dt NONE --dbsnp $db_vcf_file -o $gatk_vcf \n" ;
		print SH $cmd;
