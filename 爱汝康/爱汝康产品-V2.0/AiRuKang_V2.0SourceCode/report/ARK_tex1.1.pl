...
#	s/\r//;
	...
	$talbe_all{$h}{18} = $line[31];
   	if ($line[28] eq "ClinVar"){
			...
			$table{$i}{3} = $col_3 ;
			my ($col_4) = $line[24] =~ /c.\S+\d+(\S+)/;   #modifide
			$table{$i}{4} = $col_4;
			my $col_5;
			if ($line[25] =~ /^p/){
			    $col_5 = $line[24]." (".$line[25].")";
			}
			else{
			    $col_5 = $line[24];
			}
			...
			$table{$i}{8} = $col_8;
			$table{$i}{9} = $line[24];  #for decode
		          if ( $line[25] =~ /^p/ ) {	#for decode
			    $table{$i}{10} = $line[25];
			}
			else {
			    $table{$i}{10} = "null";
			}
			if ( $line[33]  =~ /\d+/ ){ #for decode     if ($line[33] =~ /\d+/)
			    $table{$i}{11} = "http://www.ncbi.nlm.nih.gov/clinvar/variation/$line[33]/";  #need added;
			}
			else {
			    $table{$i}{11} = "http://www.ncbi.nlm.nih.gov/clinvar/?term=$line[31]";
			}
			if ($i %2 == 0) {
