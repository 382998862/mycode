...
while(<FIN1>){
	...
	my $col_9 = $h;
	$talbe_all{$h}{9} = $col_9;
	$talbe_all{$h}{10} = $line[0];
          $talbe_all{$h}{11} = $line[1];
	$talbe_all{$h}{12} = $line[2];
	$talbe_all{$h}{13} = $line[3];
	$talbe_all{$h}{14} = $line[11];
	$talbe_all{$h}{15} = $line[4];
	my $col_16;
	if ( $line[5] =~ /\(/ ) {
	    ($col_16) = $line[5] =~ /(\S+)\(/;
	}
	...
	$talbe_all{$h}{16} = $col_16;
	$talbe_all{$h}{17} = $line[14];
	$talbe_all{$h}{18} = $line[31];
   	if ($line[28] eq "ClinVar"){
			my $i = $i++;
			my $col_1 = $i;	
			$table{$i}{1} = $col_1;
			my $col_2;
...
for (my $j=1;$j<$i;$j++) {
		print FOUT "$j.$table{$j}{2} $table{$j}{4} $table{$j}{5} \n 该变异位于$table{$j}{2}基因上，DNA编码序列变异为$table{$j}{4}，引起蛋白序列变异为$table{$j}{5}，其变异为$table{$j}{6}变异。根据权威数据库记载该变异为$table{$j}{8}。\n\n";
	}

my $decode;
for (my $j=1;$j<$i;$j++) {
		$decode .= "\\item {{\\sl{$table{$j}{2}}} $table{$j}{4} $table{$j}{5}}\n \\\\ 该变异位于{\\sl{$table{$j}{2}}}基因上,DNA编码序列变异为$table{$j}{4}，引起蛋白序列变异为$table{$j}{5},其变异为$table{$j}{6}变异。根据权威数据库记载该变异为$table{$j}{8}\\footnote{http://www.ncbi.nlm.nih.gov/clinvar/variation/0000/}。\\\n";
}

my $table_all_end;
for (my $g=1;$g<$h;$g++) {
		$table_all_end .= "$talbe_all{$g}{9} \& $talbe_all{$g}{10} \& $talbe_all{$g}{11} \& $talbe_all{$g}{12} \& $talbe_all{$g}{13} \& $talbe_all{$g}{14} \& $talbe_all{$g}{15} \& $talbe_all{$g}{16} \& $talbe_all{$g}{17} \& $talbe_all{$g}{18} \\\\  ";
	}
# $talbe_all{$g}{9}
...
\\ThisTileWallPaper{\\paperwidth}{\\paperheight}{/data/user/tengwj/30.Test/Reseq/report/Cover.pdf}
\\null
\\thispagestyle{empty}
\\clearpage


\\ThisTileWallPaper{\\paperwidth}{\\paperheight}{/data/user/tengwj/30.Test/Reseq/report/Inner.pdf}
\\vspace*{1.5cm}
\\begin{center}
...
\\TileWallPaper{\\paperwidth}{\\paperheight}{/data/user/tengwj/30.Test/Reseq/report/Inner.pdf}
\\section{检测结果}
...
{\\six\\begin{longtable}{p{1cm}p{1cm}p{2cm}p{2cm}p{2cm}p{1cm}p{1.4cm}p{1.4cm}}
...
本报告中所列信息是根据您的样本的基因突变在多个知名公共数据库及文献中查询的结果。随着科学研究的进行，某些变异的临床意义有可能发生改变。在上表中列举了引起编码序列改变的变异，完整变异情况请看附表。
...
\\section{健康管理}
\\subsection{生活建议}
鉴于您的乳腺癌遗传风险为低风险，如下方案供您参考：  %%%%%

1.提高对乳腺疾病和乳腺癌的认识，参考本报告第三部分：乳腺癌知识   %%%%%%%%%%%%%%%

2.选择健康的生活方式：

1)运动 专家推荐每周至少5天运动45-60分钟，有规律的运动可以帮助预防乳腺癌。

2)饮食美国癌症协会膳食指南建议：选择食用多种水果和蔬菜、选择全麦食物而非精制谷物、减少红肉的食用、减少酒精的消耗、增加富含钙和维生素（尤其是维生素D）食物的食用。

3)减少心理应激、增加社会支持、采取积极有效的应对方式以拮抗生活事件影响，经常保持良好、和谐的生活状态、愉快的心情。
...
\\begin{figure}[t]
		\\centering
		\\includegraphics{/data/user/tengwj/30.Test/Reseq/report/AiRuKang_fig2.pdf}
\\end{figure}
2)临床乳腺检查
...
\\section{附录：所有变异汇总}
{\\six\\begin{longtable}{p{0.7cm}p{1cm}p{1.5cm}p{0.7cm}p{0.7cm}p{1cm}p{1cm}p{1cm}p{2cm}p{1.3cm}}
\\arrayrulecolor{tabcolor}
\\toprule
\\rowcolor{linecolor1}
...
\\bottomrule
\\end{longtable}}
{\\six{Num：编号；Chrom ：染色体号；Position start ：变异起始位置；Ref：参考基因组碱基；Var:变异碱基；Region：变异碱基在基因组上的位置exonic:外显子上，intronic：内含子上，UTR3：3’端非翻译区UTR5：5’：非翻译区，upstream：上游区域；Gene：变异所处的基因；Mutation type：变异类型，synonymous SNV：同义突变，非同义突变； dbSNP：定位的SNP 号。}}
 \\clearpage
...
\\ThisTileWallPaper{\\paperwidth}{\\paperheight}{/data/user/tengwj/30.Test/Reseq/report/end.pdf}
\\null
\\clearpage
