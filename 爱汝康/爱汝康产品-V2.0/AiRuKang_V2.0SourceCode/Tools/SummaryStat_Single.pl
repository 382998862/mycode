...
my %opts;
GetOptions(\%opts, "i=s", "od=s", "p=s", "bed=s","d=i","r=s", "h");
...
print OUT "Number of reads in FASTQ_1: ", $reads_1,"\n";
print OUT "\n";
...
system("$java_dir/java -jar $gatk_dir/GenomeAnalysisTK.jar -T DepthOfCoverage -I $bam -o $outdir/$prefix.dep -L $bed -R $ref -im OVERLAPPING_ONLY -omitBaseOutput -omitLocusTable --summaryCoverageThreshold 1");
...
my $on_target = `samtools view -c $bam -L $bed -F 4`;
my @dep;
my $depth_avg;
my $coverage_avg;
