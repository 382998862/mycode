...
#&mapping_read_to_reference(\%hconfig, \%hsamples) ;

## statistic map result
#&mapping_result_stat(\%hconfig);
...
#	my %hsamples = ();
	for my $key (keys %{$ahconfig}){
		if ($key =~ /^(R\d+)$/ || $key =~ /^(R\d+L\d+)-(\d)$/){
#			$hsamples{$1}->[$2 - 1] = $ahconfig->{$key} ;
			$ahsamples->{$1} = &ABSOLUTE_DIR($ahconfig->{$key}) ;
		}
	}
	# check reads
	...
	for my $sample (keys %{$ahsamples}){
		if (!(-f $ahsamples->{$sample})){
			&show_log("ERROR:\tsample($sample) can't find file ".($ahsamples->{$sample})) ;
			exit (1);
		}
	}
	return ;
...
sub mapping_mem()
{
	...
	for my $sample (sort keys %{$ahsamples}){
		...
		print SH "/share/nas2/genome/biosoft/bwa/0.7.10/bwa mem -M -t 10 -R \"\@RG\\tID:$sample\\tLB:$sample\\tPL:ILLUMINA\\tSM:$sample_id\" " ;
		print SH " $ref", " $ahsamples->{$sample}",  ;
...
## mapping with bwa aln
sub mapping_read_to_reference()
{
	# get parameter
	my ($ahconfig, $ahsamples) = @_ ;
	my $ref = $ahconfig->{Ref} ;
	my $reflen = $ahconfig->{RefLen} ;
	# index for ref file if unindexed
	&show_log("Index for reference file.") ;
	&index_reference($ref, $reflen);
	&show_log("Index done.") ;

	# mapping reads to reference use qsub
	&show_log("Mapping reads to reference genome.") ;
	&mapping_reads($ahconfig, $ahsamples);
	&show_log("Mapping done.") ;
...
sub index_reference()
{
	my ($ref, $reflen) = @_ ;
	if (!(-f "$ref.bwt")){
		if ($reflen >= 1800000000){
			my $cmd = "bwa index -a bwtsw $ref" ;
			&run_or_die($cmd);
		}
		else{
			my $cmd = "bwa index -a is $ref" ;
			&run_or_die($cmd);
		}
	}
...
sub mapping_reads()
{
	...
	my $ref = $ahconfig->{Ref} ;
	# map
	my $map_sai_sh = "$sh_dir/$outprefix.map.sai.sh" ;
	open (SH, ">$map_sai_sh") || die "can't open $map_sai_sh, $!\n" ;
	for my $sample (sort keys %{$ahsamples}){
		if (defined $ahconfig->{Mis}){
			my $mismatch = $ahconfig->{Mis} ;
			if (defined $ahconfig->{Quality} && ($ahconfig->{Quality} == 64)){
				print SH "bwa aln $ref ", $ahsamples->{$sample}->[0], " -n $mismatch -t 3 -I -f $outdir/bwa_mid/$outprefix.$sample.1.sai\n" ;
				print SH "bwa aln $ref ", $ahsamples->{$sample}->[1], " -n $mismatch -t 3 -I -f $outdir/bwa_mid/$outprefix.$sample.2.sai\n" ;
			}
			else{
				print SH "bwa aln $ref ", $ahsamples->{$sample}->[0], " -n $mismatch -t 3 -f $outdir/bwa_mid/$outprefix.$sample.1.sai\n" ;
				print SH "bwa aln $ref ", $ahsamples->{$sample}->[1], " -n $mismatch -t 3 -f $outdir/bwa_mid/$outprefix.$sample.2.sai\n" ;
			}
		}
		else{
			if (defined $ahconfig->{Quality} && ($ahconfig->{Quality} == 64)){
				print SH "bwa aln $ref ", $ahsamples->{$sample}->[0], " -t 3 -I -f $outdir/bwa_mid/$outprefix.$sample.1.sai\n" ;
				print SH "bwa aln $ref ", $ahsamples->{$sample}->[1], " -t 3 -I -f $outdir/bwa_mid/$outprefix.$sample.2.sai\n" ;
			}
			else{
				print SH "bwa aln $ref ", $ahsamples->{$sample}->[0], " -t 3 -f $outdir/bwa_mid/$outprefix.$sample.1.sai\n" ;
				print SH "bwa aln $ref ", $ahsamples->{$sample}->[1], " -t 3 -f $outdir/bwa_mid/$outprefix.$sample.2.sai\n" ;
			}
		}
	}
	close(SH);
	...
	# convert to sam file
	my $map_sam_sh = "$sh_dir/$outprefix.map.sam.sh" ;
	open (SH, ">$map_sam_sh") || die "Can't open $map_sam_sh, $!\n" ;
	my ($min_insert_size, $max_insert_size) = split /\-/, $ahconfig->{Ins} ;
	for my $sample (sort keys %{$ahsamples}){
		my $sample_id = $sample ;
		$sample_id = $1 if ($sample =~ /(R\d+)L\d+/) ;
		print SH "bwa sampe -a $max_insert_size -r \"\@RG\tID:$sample\tLB:$sample\tPL:ILLUMINA\tSM:$sample_id\"" ;
		print SH " $ref $outdir/bwa_mid/$outprefix.$sample.1.sai $outdir/bwa_mid/$outprefix.$sample.2.sai ", $ahsamples->{$sample}->[0], " ", $ahsamples->{$sample}->[1] ;
		print SH " | gzip > $outdir/bwa_mid/$outprefix.$sample.sam.gz\n" ;
	}
	close(SH);
	&qsub($map_sam_sh);
...
sub mapping_result_stat()
{
	...
	my $chr_num = $ahconfig->{ChrNum} ;
	my $ref = $ahconfig->{Ref} ;
	my $ref_len_file = "$ref.len" ;
	my $ref_dir = dirname($ref) ;
	if (!-f $ref_len_file){
		my $cmd = "/share/nas2/genome/biosoft/perl/5.18.2/bin/perl $Bin/../Tools/ref_GC_len.pl -ref $ref -od $ref_dir" ;
		&run_or_die($cmd);
	}
	my $cmd = "perl $Bin/Map_stat.pl -id $indir -od $dir -p $outprefix -rl $ref_len_file -n $chr_num -m $maxproc" ;
	&run_or_die($cmd);
...
sub Summary_mapping()
{
	...
	for my $sample (sort keys %{$ahsamples}){
		...
		print SH "perl /share/nas17/macc/Reseq/Tools/SummaryStat_Single.pl -i $BAM -r $ref -od $od -bed $BED -p $outprefix.$sample -d $Depth \n";
	}
	close(SH);
