...
... ){
	...
	plot(x, y1, pch=line.pch[1], lwd=line.lwd[1], 
	...
	axis(2, ylim=c(y1.min.limit,y1.max.limit),
		col="black", col.axis="black", las=1,
		lwd=axis.lwd, cex.axis=axis.cex) ## las=1 makes horizontal labels
	mtext(y1.lab, side=2, line=text.line[2], col="black", cex=text.cex)
	box(lwd=axis.lwd)
	...
	plot(x, y2, pch=line.pch[2], lwd=line.lwd[2],
		axes=FALSE, xlim=c(x.min.limit,x.max.limit),
		ylim=c(y2.min.limit,y2.max.limit), 
		xlab="", ylab="", type="o",col=line.col[2])
	## a little farther out (line=4) to make room for labels
	axis(4, ylim=c(y2.min.limit,y2.max.limit),
		col="black", col.axis="black", las=1, 
		lwd=axis.lwd, cex.axis=axis.cex) ## las=1 makes horizontal labels
	mtext(y2.lab, side=4, line=text.line[3], col="black", cex=text.cex)
	...
	legend(x.pos, y.pos, legend=c(y1.lab,y2.lab),
		text.col=line.col, lty=c(1,1), pch=line.pch, 
		col=line.col, lwd=line.lwd[1], cex=legend.cex, box.lwd=legend.box.lwd)
