...
sub mapping_mem()
{
	...
	for my $sample (sort keys %{$ahsamples}){
		...
		print SH "/share/nas2/genome/biosoft/bwa/bwa-0.7.1/bwa mem -M -t 10 -R \"\@RG\tID:$sample\tLB:$sample\tPL:ILLUMINA\tSM:$sample_id\" " ;
	...
	for my $sample (sort keys %{$ahsamples}){
		if (defined $ahconfig->{Mis}){
			...
			if (defined $ahconfig->{Quality} && ($ahconfig->{Quality} == 64)){
				print SH "bwa aln $ref ", $ahsamples->{$sample}->[0], " -n $mismatch -t 30 -I -f $outdir/bwa_mid/$outprefix.$sample.1.sai\n" ;
				print SH "bwa aln $ref ", $ahsamples->{$sample}->[1], " -n $mismatch -t 30 -I -f $outdir/bwa_mid/$outprefix.$sample.2.sai\n" ;
			}
			else{
				print SH "bwa aln $ref ", $ahsamples->{$sample}->[0], " -n $mismatch -t 30 -f $outdir/bwa_mid/$outprefix.$sample.1.sai\n" ;
				print SH "bwa aln $ref ", $ahsamples->{$sample}->[1], " -n $mismatch -t 30 -f $outdir/bwa_mid/$outprefix.$sample.2.sai\n" ;
			}
		}
		else{
			if (defined $ahconfig->{Quality} && ($ahconfig->{Quality} == 64)){
				print SH "bwa aln $ref ", $ahsamples->{$sample}->[0], " -t 30 -I -f $outdir/bwa_mid/$outprefix.$sample.1.sai\n" ;
				print SH "bwa aln $ref ", $ahsamples->{$sample}->[1], " -t 30 -I -f $outdir/bwa_mid/$outprefix.$sample.2.sai\n" ;
			}
			else{
				print SH "bwa aln $ref ", $ahsamples->{$sample}->[0], " -t 30 -f $outdir/bwa_mid/$outprefix.$sample.1.sai\n" ;
				print SH "bwa aln $ref ", $ahsamples->{$sample}->[1], " -t 30 -f $outdir/bwa_mid/$outprefix.$sample.2.sai\n" ;
			}
		}
	}
	close(SH);
...
sub mapping_result_stat()
{
	...
	if (!-f $ref_len_file){
		my $cmd = "perl $Bin/../Tools/ref_GC_len.pl -ref $ref -od $ref_dir" ;
		&run_or_die($cmd);
	}
...
sub Summary_mapping()
{
	...
	for my $sample (sort keys %{$ahsamples}){
		...
		print SH "perl $Bin/../Tools/SummaryStat_Pair_v2.0.pl -i $BAM -r $ref -od $od -bed $BED -p $outprefix.$sample -d $Depth \n";
	}
	close(SH);
