...
sub create_work_shell{
	...
	foreach my $sample_name (sort keys %$SamData) {
		...
		if( exists $SamData->{"$sample_name"}->{"2"} ){
			...
			print "fq1: $fq1\n";
			print "fq2: $fq2\n";
			print OUT "cd  $od && $fastq_qc_stat -Q $Q -a $fq1 -b $fq2 -f $sample_name && ";
		# for single sequencing
		} else {
			my $fq1 = $SamData->{"$sample_name"}->{"1"};
			print OUT "cd  $od && $fastq_qc_stat -Q $Q -a $fq1 -f $sample_name && ";
		}
		# drawing cmd
...
sub calling_qsub{
	...
	# calling qsub
	`nohup $qsubcmd  $od/work_sh/GC_Q_svg.sh --reqsub  --queue $queue --maxproc $qsubNum `;
}
